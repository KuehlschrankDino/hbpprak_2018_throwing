...
def mutatePopulation(pop, local_max_counter,current_gen,weights_mapping):
    ...
    for i, p in enumerate(pop):
        ...
        if (local_max_counter == 10 and i < 10):
            ...
            new_weights_mapping.append({
                    ...
                    'oldweights': np.zeros(WEIGHT_SHAPE[0],WEIGHT_SHAPE[1]),
                    'distance': 0
                    })
        elif(i < 10):
            for ii in range(3): 
            ...
            new_weights[i, ]= pop[value]['weights']+np.random.rand(1) * np.power(-1, np.random.randint(2)) * LR
            new_weights_mapping.append({
                    'newweights': new_weights[i],
    ...
    if(current_gen> 0 and current_gen %10 == 0) :
        ...
        for i, p in enumerate(pop):
            ...
            print("#################oldweights -{}".format(old_weights))
            dif =pop[i]['weights']-old_weights['oldweights']
            norm_dif = dif /dif.sum(axis=1,keepdims=1)
...
def searchForOldWeights(weights_mapping, new_weights):
    for i,p in enumerate(weights_mapping):
        print("#################newweights -{}".format(new_weights))
        print("#################oldweights -{}".format(weights_mapping[i]['newweights']))
